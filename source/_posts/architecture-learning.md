...
系统隔离可以在系统发生故障时，能限定传播范围和影响范围，以保证只有出问题的服务不可用而不是影响系统其他部分的使用。

资源隔离可以减少资源竞争，保障服务间的独立性和可用性。

我们常见的隔离手段有线程隔离、集群隔离、机房隔离、读写隔离、快慢隔离、动静隔离、爬虫隔离等。简单说，隔离就是对一个大的进行分割，已确保每一部分都是独立而不互相影响的，从而保障了系统的稳定性和高可用性。
