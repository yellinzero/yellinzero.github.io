...
代理层的超时设置只是一个常见的应用场景，但实际上，从网络请求到数据存储，每个环节都需精心设计超时和重试机制。这包括但不限于容器化应用、中间件服务、数据库访问，乃至业务逻辑处理。

##### 关键考虑点

- **容器与微服务**: 在容器化和微服务架构中，网络请求可能需要经过多个服务节点。为每个服务定义合理的超时和重试策略，能够有效减少网络延迟和服务熔断的风险。

- **中间件**: 对于如消息队列、缓存等中间件服务，需要根据其特性和应用场景配置超时参数，以及在服务不可用时的重试逻辑。

- **数据库访问**: 数据库操作往往是业务流程中的关键步骤。为数据库读写设置适当的超时限制，可以避免单一慢查询影响到整个应用性能。

- **业务逻辑处理**: 业务层面的超时和重试策略也同样重要，特别是在处理外部API调用或复杂计算任务时。

##### 实施建议

- **分层设定**: 根据应用架构的不同层次，分别考虑和设定超时和重试策略，确保每一层都有针对性的保护措施。

- **动态调整**: 在可能的情况下，采用动态调整超时和重试参数的策略，根据系统的当前负载和性能自动调节。

- **故障隔离**: 通过超时和重试策略的合理应用，实现故障隔离，避免单点故障影响到整个系统。

- **监控与报警**: 结合系统监控和报警工具，实时跟踪超时和重试事件，及时发现和处理潜在问题。

### 回滚机制

#### 数据一致性

在深入探讨回滚机制之前，先了解两种分布式系统中数据一致性模型的概念：最终一致性（Eventual Consistency）和强一致性（Strong Consistency）。这两个术语描述了系统在数据更新和访问时，维持一致性的不同方法及其保证的程度。

##### 强一致性

强一致性，亦称为严格一致性，指的是系统在完成数据更新之后，任何随后的数据访问都将立刻反映该更新结果。这一模型下，系统对外的行为仿佛操作都在单一节点上执行，确保了更新之后的所有读操作均返回最新值。

强一致性的特征包括：

- **即时可见性**：数据更新后，所有读操作立刻展现新的数据状态。
- **直观性**：对用户和开发者而言，强一致性模型易于理解，因其符合更新后立即可见的直觉逻辑。
- **成本与可扩展性**：实现强一致性可能需采用复杂的同步机制，这可能对系统性能和扩展性产生影响。

例如，使用事务的关系型数据库和配置为强一致性的分布式数据库系统（如Google Spanner）。

##### 最终一致性

相比之下，最终一致性提供了更为宽松的一致性保证。它允许在数据更新后的一段时间内，更新结果对所有访问不立即可见。换言之，更新后的读操作可能短暂看不到最新值。但系统保证，若无新更新产生，终将达到一个状态，使所有读操作能获取到最新更新结果。

最终一致性的特征包括：

- **可扩展性**：由于无需即时进行跨节点同步，最终一致性模型更易于扩展。
- **性能优势**：减少同步等待时间，提升系统响应和吞吐量。
- **管理复杂性**：可能增加理解和处理不同时点看到的不同数据状态的难度。

典型应用包括多数NoSQL数据库（如Cassandra和DynamoDB）及分布式缓存系统。

##### 强一致性与最终一致性的区别

核心区别在于数据更新后的可见性保障：

- **强一致性**确保更新后任何读取操作都能立刻看到最新数据。
- **最终一致性**允许短暂时间窗口内读取操作可能未见最新数据，但最终所有读取都会反映更新。

#### 事务回滚

单个事务的回滚只要使用数据库提供的事务回滚api即可，比较麻烦的是分布式事务的回滚，这时通常只能保证最终一致性，而无法做到强一致性。

考虑到电商平台下单过程，可能涉及到商品服务、订单服务和支付服务。使用分布式事务框架（如Seata）可协调这些服务间的操作，以保障业务流程的完整性和一致性。通过二阶段提交机制，确保所有相关操作要么全部成功，要么在任何服务执行失败时全部回滚。

##### 场景描述

用户下单购买商品，这个过程涉及到三个步骤：
...
假设我们使用了像 Seata 这样的分布式事务框架来管理跨服务的事务。

1. **事务协调**：当用户下单时，事务协调器（TC）开始一个全局事务。

2. **分支事务**：
   
   - 商品服务减少库存，并注册一个分支事务到 TC。
   - 订单服务创建订单，并注册一个分支事务到 TC。
   - 支付服务扣款，并注册一个分支事务到 TC。

3. **二阶段提交**：
   
   - **预提交**：每个服务都尝试执行其操作，并报告其成功或失败状态给 TC。
   - **提交或回滚**：如果所有服务都报告成功，TC 则指示所有服务提交事务。如果任何服务报告失败，TC 则指示所有服务回滚事务。

##### 案例分析

假设在扣款操作时发现用户余额不足，支付服务无法完成扣款。支付服务报告操作失败给事务协调器。事务协调器收到失败消息后，决定回滚整个全局事务，它会指示商品服务回滚库存增加操作，指示订单服务取消订单。

通过这种方式，即使在分布式环境中发生故障，系统也能保证最终一致性，确保要么所有相关操作全部成功完成（用户成功下单），要么全部回滚（订单取消，库存恢复，不扣款）。

#### 回滚策略的其他维度

- **代码回滚**: 遵循版本控制系统的规范，确保代码更改可追溯并容易回滚到稳定版本。

- **部署版本回滚**:
  
  - **版本化部署**: 每次部署保留前一版本包，以便于全量回滚。
  - **小版本增量发布**: 对小功能更新先在单节点测试，然后逐渐扩大部署范围。
  - **大版本灰度发布**: 新旧版本并行，全面验证后全量部署新版本。
  - **架构升级并发发布**: 类似于大版本灰度，但更侧重于数据迁移和功能验证。

- **数据版本回滚**: 实施数据备份策略，全量或增量备份，确保数据可靠性。

- **静态资源版本回滚**: 通过加入hash值或版本号管理静态资源，简化回滚流程，避免CDN缓存问题。
...
缓存是提升系统性能和高可用性的关键技术。合理的缓存回收算法能够确保缓存系统有效运行，防止内存溢出。以下是几种常用的缓存回收算法：

- **FIFO（First In First Out）**: 先进先出算法，按照缓存对象进入缓存的顺序进行回收，最早添加到缓存的对象首先被移除。

- **LRU（Least Recently Used）**: 最近最少使用算法，优先回收最长时间未被访问的缓存对象，广泛应用于实际场景中。

- **LFU（Least Frequently Used）**: 最少使用算法，根据对象在一定时间内的访问频率进行回收，频率最低的缓存对象首先被移除。

#### Java中的缓存类型及实现

Java中的缓存类型多样，根据存储位置和持久化需求的不同，可以选择不同类型的缓存实现策略：

- **堆缓存**: 利用Java堆内存存储缓存对象，提供快速的读写访问。通常使用软引用/弱引用存储缓存对象以便于垃圾回收。

- **堆外缓存**: 存储在Java堆外内存中，避免了堆缓存可能导致的垃圾回收问题，但需要考虑对象的序列化/反序列化开销。

- **磁盘缓存**: 通过持久化到磁盘，提供了持久化缓存解决方案，能够在系统重启后保持数据不丢失。

- **分布式缓存**: 在分布式系统中，缓存数据分布在多个节点上，重点考虑数据一致性问题。分布式缓存能够提高系统的扩展性和可用性，常见的分布式缓存解决方案包括Redis和Memcached。

缓存类型的选择取决于应用场景的具体需求，如性能、数据一致性、持久化等。合理地使用缓存和选择适合的缓存回收算法，是提升应用性能和可用性的有效手段。

#### 缓存使用模式实践

应用级缓存实践通常采取两种模式：**Cache-Aside（旁路缓存）** 和**Cache-As-SoR（缓存作为数据源）**。

我们先了解三个常见概念

- **SoR(System-of-record):** 记录系统，即数据源
...
- **读取操作**: 首先尝试从缓存获取数据，未命中则从数据源（SoR）获取并缓存。
- **写入操作**: 先写数据源，成功后再更新缓存或使缓存失效。

##### Cache-As-SoR

- **Read-Through**: 未命中缓存时，由缓存层自动回源加载数据。
- **Write-Through**: 写操作直接对缓存进行，由缓存层负责同步更新数据源。
- **Write-Behind**: 写操作异步同步到数据源，支持批处理、合并写操作。

##### Cache-As-SoR

简单点说，就是从业务看来把Cache作为SoR来操作，所有操作都是在Cache进行的，然后又Cache根据情况去操作SoR，即在业务代码中只能看到操作Cache的逻辑。通常有3种模式：

- **Read-Through:** 业务代码首先调用Cache，如果Cache不命中由Cache回源到SoR，而不是业务代码。使用此模式需要配置一个中间件来回源到SoR加载源数据。

- **Write-Through:** 被称为穿透写模式/直写模式，即业务代码首先调用Cache写（新增/修改）数据，然后由Cache负责写缓存和写SoR。使用此模式也需要配置相关中间件。

- **Write-Behind:** Write-Behind也叫做Write-Back，即回写模式。不同于Write-Through时同步写SoR和Cache，此模式是异步操作，异步之后还可以实现批量写、合并写、延时和限流。

##### Copy Pattern

分别有 **Copy-On-Read(在读时复制)** 和 **Copy-On-Write(在写时复制)** 。

缓存策略的选择和实施需根据应用的具体需求和特点进行。通过合理的缓存设计，可以显著提升应用的性能和用户体验。



### HTTP缓存

HTTP缓存是一种重要的网络技术，用于减少服务器负载、降低响应时间并提升用户体验。

#### 关键缓存字段

- **Last-Modified**: 指示资源最后一次被修改的时间。客户端可以在后续请求中使用`If-Modified-Since`头部，以此时间为依据决定是否需要重新获取资源。

- **Expires**: 明确指定资源的过期时间。一旦资源过期，缓存将被视为失效，客户端需要向服务器重新请求资源。

- **Cache-Control**: 是一个非常灵活的HTTP头部，用于定义资源的缓存策略。常见指令包括`max-age`、`no-cache`、`no-store`、`must-revalidate`等，控制资源的缓存周期和验证方式。

- **ETag**: 资源的唯一标识符（实体标签）。常用于验证缓存的新鲜度，服务器利用ETag检查资源是否有更新。

- **Age**: 表示资源在缓存中存储了多长时间，通常由缓存服务器添加。

- **Vary**: 指示代理服务器缓存多个版本的响应，基于请求头字段的值（如`Accept-Language`），来处理对同一资源的不同请求。

- **Via**: 显示了请求和响应经过的中间代理。这个头部可以帮助识别请求/响应链路上的问题，同时确保缓存透明性。

#### 常用缓存操作

- **F5**: 在大多数浏览器中，按F5会刷新页面，这通常导致浏览器进行条件性请求，检查缓存资源是否仍然有效（例如，使用`If-Modified-Since`和`ETag`）。

- **Ctrl+F5**: 强制浏览器忽略缓存，直接从服务器重新下载所有资源。这对于开发和调试非常有用，可以确保从服务器获取最新的资源，而不是使用过时的本地缓存。
